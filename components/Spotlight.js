export default function Spotlight({ image, artist }) {
  ...
  // getRandomArt will give a key of artGalley array
  function getRandomArt(totalArts) {
    artKey = Math.floor(Math.random() * totalArts);
  }

  console.log(artGallery[artKey].name);
  console.log(artGallery[artKey].imageSource);
  // const getArt = artGallery.filter((art) => art.id === artKey);
  //console.log(getArt);
  ...
  getRandomArt(totalArts);

  console.log(artKey);
  return (
    <>
      <h1>Artist Name : {artGallery[artKey].name}</h1>
    </>
  );
}
